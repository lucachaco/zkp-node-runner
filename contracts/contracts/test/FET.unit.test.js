...
contract('FET', function([_, initialHolder, recipient, anotherAccount]) {
  const initialSupply = new BN(100);
  beforeEach(async function() {
    this.token = await FET.new('FungibleEventToken', 'FET', 2, initialHolder, initialSupply);
  });
    ...
    describe('when the recipient is not the zero address', function() {
      ...
      describe('when the spender has enough approved balance', function() {
        ...
        describe('when the initial holder has enough balance', function() {
          ...
          it('emits a transfer event', async function() {
            const { logs } = await this.token.transferFrom(initialHolder, to, amount, {
              from: spender,
            });
  ...
  describe('safe transfer from', function() {
    const spender = recipient;
    const encode = abi.rawEncode(['uint256'], ['23456787']);
    const data = `0x${encode.toString('hex')}`;
    describe('when the recipient is an IOCTReceiver contract', function() {
      let to;

      beforeEach(async function() {
        const OCTReceiverContract = await OCTReceiverMock.new();
        to = OCTReceiverContract.address;
      });
      ...
      describe('when the spender has enough approved balance', function() {
        ...
        describe('when the initial holder has enough balance', function() {
          ...
          it('transfers the requested amount', async function() {
            await this.token.safeTransferFrom(initialHolder, to, amount, data, { from: spender });
          ...
          it('emits a transfer event', async function() {
            const { logs } = await this.token.safeTransferFrom(initialHolder, to, amount, data, {
              from: spender,
            });
        ...
        describe('when the initial holder does not have enough balance', function() {
          ...
          it('reverts', async function() {
            await shouldFail.reverting(
              this.token.safeTransferFrom(initialHolder, to, amount, data, { from: spender }),
            );
          });
        });
      });
    ...
    describe('when the recipient is not a contract and not the zero address', function() {
      const to = anotherAccount;
      ...
      describe('when the sender has enough balance', function() {
        ...
        describe('when the spender had an approved amount', function() {
          ...
          it('increases the spender allowance adding the requested amount', async function() {
            ...
            (await this.token.allowance(initialHolder, spender)).should.be.bignumber.equal(
              amount.addn(1),
            );
          });
        });
  ...
  describe('split', function() {
    const val30 = new BN(30);
    const val10 = new BN(10);
    const val20 = new BN(20);
    const val100 = new BN(100);

    describe('when length of recepient and split share does not match', async function() {
      it('reverts', async function() {
        await shouldFail.reverting(
          this.token.split(initialHolder, [recipient, anotherAccount], [val10], val30, val100),
        );
      });
    });

    describe('when length of recipients is zero', async function() {
      it('reverts', async function() {
        await shouldFail.reverting(this.token.split(initialHolder, [], [], val30, val100));
      });
    });

    describe('when sum of numerator does not match denominator', async function() {
      it('reverts', async function() {
        await shouldFail.reverting(
          this.token.split(
            initialHolder,
    ...
    describe('when all conditions are met for one recipient', async function() {
      it('emits events', async function() {
        const { logs } = await this.token.split(initialHolder, [recipient], [val10], val10, val100);
    ...
    describe('when all conditions are met for multiple recipients', async function() {
      it('emits events', async function() {
